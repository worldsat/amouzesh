    ...
    private void getData() {
        ...
        controller().GetFromApi2(address, new CallbackGetString() {
            ...
            public void onSuccess(String resultStr) {
                try {
                    GetRelatedComment getRelatedComment = gson().fromJson(resultStr, GetRelatedComment.class);

                    if (getRelatedComment.getData().size() > 0) {
                        warningTxt.setVisibility(View.GONE);
                        progressBar.setVisibility(View.GONE);
                        recyclerViewlistPost.setVisibility(View.VISIBLE);

                        adapter = new CommentListAdapter(getRelatedComment.getData());
...
//        floatingActionButton1 = findViewById(R.id.material_design_floating_action_menu_item1);
    ...
    private void bottomView() {
        ...
        btn1.setOnClickListener(v -> {
            Intent intent = new Intent(ListCommentActivity.this, ProfileActivity.class);
        ...
        btn4.setOnClickListener(v -> {
            Intent intent = new Intent(ListCommentActivity.this, ListPostActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn5.setOnClickListener(v -> {
            Intent intent = new Intent(ListCommentActivity.this,BookmarkListActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
