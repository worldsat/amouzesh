...
import com.atrinfanavaran.school.Adapter.New.AnnouncementListAdapter;
...
public class ListAnnouncementActivity extends BaseActivity {
    ...
    private void getData() {
        ...
        CategoryId = getIntent().getIntExtra("CategoryId", 0);
            ...
            address = "api/Announcement/GetAll?Id=" + settingsBll().getApplicationUserId();
        ...
        controller().GetFromApi2(address, new CallbackGetString() {
            ...
            public void onSuccess(String resultStr) {
                try {
                    AnnouncementGetAll announcementGetAll = gson().fromJson(resultStr, AnnouncementGetAll.class);

                    if (announcementGetAll.getData().size() > 0) {
                        warningTxt.setVisibility(View.GONE);
                        progressBar.setVisibility(View.GONE);
                        recyclerViewlistPost.setVisibility(View.VISIBLE);

                        adapter = new AnnouncementListAdapter(announcementGetAll.getData());
            ...
            public void onError(String error) {
                Toast.makeText(ListAnnouncementActivity.this, error, Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void setVariable() {
        titleTxt.setText("اعلانات");
        recyclerViewlistPost.setLayoutManager(new LinearLayoutManager(this));

        floatingActionButton1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(ListAnnouncementActivity.this, SendAnnouncementActivity.class));
            }
        });
    }
...
//        view4.setVisibility(View.VISIBLE);
        ...
        btn1.setOnClickListener(v -> {
            Intent intent = new Intent(ListAnnouncementActivity.this, ProfileActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn2.setOnClickListener(v -> {
            Intent intent = new Intent(ListAnnouncementActivity.this, Main2Activity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn3.setOnClickListener(v -> {
            Intent intent = new Intent(ListAnnouncementActivity.this, Main3Activity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn4.setOnClickListener(v -> {
            Intent intent = new Intent(ListAnnouncementActivity.this, ListAnnouncementActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn5.setOnClickListener(v -> {
            Intent intent = new Intent(ListAnnouncementActivity.this,BookmarkListActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
