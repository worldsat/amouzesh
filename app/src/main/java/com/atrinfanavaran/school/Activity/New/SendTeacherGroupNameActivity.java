...
public class SendTeacherGroupNameActivity extends BaseActivity {
    ...
    private void setVariable() {
        titleTxt.setText("گروه دبیران");

        saveBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (edt1.getText().toString().trim().isEmpty()) {
                    Toast.makeText(SendTeacherGroupNameActivity.this, "لطفا عنوان را وارد نمائید", Toast.LENGTH_SHORT).show();
                } else {
                    Controller controller = new Controller(SendTeacherGroupNameActivity.this);
                    SettingsBll settingsBll = new SettingsBll(SendTeacherGroupNameActivity.this);

                    try {
                        params.put("Name", edt1.getText().toString().trim());
                        params.put("ApplicationUserId", settingsBll.getApplicationUserId());
                        params.put("IsForTeacher", true);
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }

                    Log.i(TAG, "onClick: " + params.toString());
                    MaterialDialog wait=new Waiting(SendTeacherGroupNameActivity.this).alertWaiting();
                    wait.show();
                    controller().operationProcess(SendTeacherGroupNameActivity.this, "api/CustomGroup/Add", params.toString(), new CallbackOperation() {
                        @Override
                        public void onSuccess(String result) {
                            try {
                                ...
                                Toast.makeText(SendTeacherGroupNameActivity.this, "" + e.toString(), Toast.LENGTH_SHORT).show();
                            }
                            wait.dismiss();
                        }
                        ...
                        public void onError(String error) {
                            wait.dismiss();
                            Toast.makeText(SendTeacherGroupNameActivity.this, "" + error.toString(), Toast.LENGTH_SHORT).show();
                        }
                    });
    ...
    private void bottomView() {
        ...
        btn1.setOnClickListener(v -> {
            Intent intent = new Intent(SendTeacherGroupNameActivity.this, ProfileActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn2.setOnClickListener(v -> {
            Intent intent = new Intent(SendTeacherGroupNameActivity.this, Main2Activity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn3.setOnClickListener(v -> {
            Intent intent = new Intent(SendTeacherGroupNameActivity.this, Main3Activity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn4.setOnClickListener(v -> {
            Intent intent = new Intent(SendTeacherGroupNameActivity.this, ListPostActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
        btn5.setOnClickListener(v -> {
            Intent intent = new Intent(SendTeacherGroupNameActivity.this,BookmarkListActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            startActivity(intent);
            overridePendingTransition(0, 0); //0 for no animation
        });
